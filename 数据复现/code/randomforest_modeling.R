...
samples_env_info <- fread("~/samples.env.info",header = F)
...
# fwrite(df_0,"~/df_412_terra.csv")

## samples binding environmental variables raster=======================================
...
# fwrite(test,"/mapped_df_412_terra.csv")
...
set.seed(1,"L'Ecuyer-CMRG")
inTrain <- createDataPartition(df0$nif_abd, p = 0.8, list = FALSE)[,1]   
data_train <- df0[inTrain,]
data_test <- df0[-inTrain,]
...
control_grid <- trainControl(method = "cv", number = 10)
...
### 10 random forest for final prediction
#require(terra)
...
# fwrite(predictsum,"~/rproject/new_way/mean_type_terra_final_10_cv_0.05.csv")
